...
object J003 {
    ...
    private const val WIRE0 = 0x01
    private const val WIRE1 = 0x02

    private const val LEFT = "L"
    private const val UP = "U"
    private const val DOWN = "D"
    private const val RIGHT = "R"
    private val REGEX = """([$LEFT$RIGHT$UP$DOWN])(\d+)""".toRegex()
    ...
    fun minDistance(input: String): Int {
        val map = computeMap(input)


        return map.entries.filter { (_, wires) ->
            wires.wires == WIRE0 or WIRE1
        }.map { (position, _) ->
            val (x, y) = position
            abs(x) + abs(y)
        }.min() ?: 0
    ...
    private fun computeMap(input: String): MutableMap<Pair<Int, Int>, Metadata> {
        val map = mutableMapOf<Pair<Int, Int>, Metadata>()

        val paths = input.split("\n")

        computeWirePositions(paths, map, 0, WIRE0)
        computeWirePositions(paths, map, 1, WIRE1)
        return map
    }


    private fun computeWirePositions(
        paths: List<String>,
        map: MutableMap<Pair<Int, Int>, Metadata>,
        wireIndex: Int,
        wireId: Int
    ) {
        var currentPosition = 0 to 0
        var steps = 0
        paths[wireIndex].split(",").map { instruction ->
            val (direction, count) = REGEX.matchEntire(instruction)!!.destructured
            val (x, y) = currentPosition

            for (delta in 1..count.toInt()) {
                ...
                steps++
                val data = Metadata(wireId, steps)
                map.merge(currentPosition, data) { prev, new ->
                    if (prev.wires and wireId == wireId) {
                        prev
                    } else {
                        val wires = prev.wires or new.wires
                        val newSteps = prev.steps + new.steps
                        Metadata(wires, newSteps)
                    }
                }
            }
        }
