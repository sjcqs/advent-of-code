...
internal class J007Test {
    @ParameterizedTest
    @MethodSource("intCodes")
    internal fun testMaxOrder(value: String, expected: Int) {
        assertEquals(expected, J007.findMaxOrder(value, J007.Mode.SIMPLE))
    }

    @ParameterizedTest
    @MethodSource("inCodesLoop")
    internal fun testMaxOrderLoop(value: String, expected: Int) {
        assertEquals(expected, J007.findMaxOrder(value, J007.Mode.LOOP))
    }

    companion object {
        @JvmStatic
        fun intCodes() = Stream.of(
        ...
        @JvmStatic
        fun inCodesLoop() = Stream.of(
            Arguments.of("3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26," +
